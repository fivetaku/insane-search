...
**예상 동작:** Claude가 각 사이트의 공개 경로 — 레딧 피드, X의 무료 Brave·Yahoo 검색과 tweet-result 검증, 유튜브 자막 — 로 로그인·API 키 없이 도달한다. xAI 자격정보가 이미 있으면 X 키워드 검색에 네이티브 `x_search`가 자동 병합되며, 무료 경로는 계속 유지된다. `INSANE_SEARCH_XAI=off`로 무료 경로만 강제할 수도 있다.
